...
def start_worker():
    ...
    server_add = '127.0.0.1'    # 服务器地址
    server_port = 5000          # 服务器端口
    print('Connect to server %s...' % server_add)
    # 端口和验证码需要保持与task_master.py中设置的完全一致
    manager = QueueManager(address=(server_add, server_port), authkey=b'abc')
    # 从网络连接
    manager.connect()
    ...
    while True:
        try:
            ...
            print('Get task %d: calculate %d + %d = ?' % (n, n, n))
            time.sleep(1)
            r = '%d + %d = %d' % (n, n, n + n)
            print('task %d run over' % n)
            result.put(r)
        except queue.Empty:
